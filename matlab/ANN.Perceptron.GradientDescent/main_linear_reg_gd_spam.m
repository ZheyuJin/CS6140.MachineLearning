...
for curFold  = 0 : kfold -1
    ...
    %jump into GD..!!
    w = linear_gd_batch_update(1, w, trainData, labelTrain, 1000);
    
    %train MSE
    outTrain = trainData * w;
...
%plotting roc.
totalLabelLinear = [labelTrain; labelTest];
totalOutLinear = [outTrain; outTest];
plotroc(totalLabelLinear',totalOutLinear');

%compute AUC 
[X,Y,T,LinearAUC,OPTROCPT,SUBY,SUBYNAMES] = perfcurve(totalLabelLinear',totalOutLinear',1);
LinearAUC
