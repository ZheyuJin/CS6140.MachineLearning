...
for curFold  = 0 : kfold -1
    ...
    % calc mean for two classes in tranning data. both 1*57 
    [meanSpamVec, meanNonSpamVec] = calcMean_eachfeature(trainData, trainLabel);
    
    % find cov for trannig data.
    std_dev = std(trainData); %standard deviation. 1 * 57
    
    % find err for test data.
    [testOut, rawGDAOut] = predict_GDA(priorSpam, priorNonSpam, meanSpamVec, meanNonSpamVec, std_dev, testData);
