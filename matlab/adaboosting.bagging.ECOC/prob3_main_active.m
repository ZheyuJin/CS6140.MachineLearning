% active learning vs random data point picking.

%% initial data loading for active learning
...
trainSet = trainSet(~initialTrainRows ,:); %size shrink

testErrContainer = [];
ADA_MAX_ROUND = 10;
%% loop for actrive learning 
...
while true
    ...
    dist = abs(rawOutput); % dist to the hyper plane F(X)= 0;
%     maxraw = max(rawOutput)
%     minraw = min(rawOutput)
    [~,index] = sort(dist);
